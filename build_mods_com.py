...
def build_mods_com():
    ...
    # easy_supers: per-handler 75-byte patches
    # Bytes 20-21 are an MZ relocation target → JMP SHORT skips them.
    # Uses CALL NEAR (E8 rel16) to execute_super, matching original convention.
    # Each handler needs its own displacement since they're at different offsets.
    #   P1: Q=0x10 E=0x12 Z=0x2C   P2: U=0x16 O=0x18 M=0x32
    #   P3: Num7=0x47 Num9=0x49 Num1=0x4F
    easy_super_prefix = [                    # bytes 0-54 (common)
         0x55,                               # 0:  push bp
         0x8B, 0xEC,                         # 1:  mov bp, sp
         0x56,                               # 3:  push si
         0x57,                               # 4:  push di
         0x8B, 0x76, 0x06,                   # 5:  mov si, [bp+6]
         0x8B, 0x7E, 0x08,                   # 8:  mov di, [bp+8]
         0x83, 0xFE, 0x03,                   # 11: cmp si, 3
         0x7D, 0x2E,                         # 14: jge done  (+46 → byte 62)
         ...
         0xB3, 0x03,                         # 24: mov bl, 3
         0xF6, 0xE3,                         # 26: mul bl   (AX = si*3)
         0x03, 0xC7,                         # 28: add ax, di
         0xE8, 0x00, 0x00,                   # 30: call $+3  (PIC: push IP)
         0x5B,                               # 33: pop bx
         0x83, 0xC3, 0x21,                   # 34: add bx, 33  (bx → table@66)
         0x2E, 0xD7,                         # 37: cs xlatb  (al = scancode)
         0x30, 0xE4,                         # 39: xor ah, ah
         0x8B, 0xD8,                         # 41: mov bx, ax
         0xD1, 0xE3,                         # 43: shl bx, 1
         0x83, 0xBF, 0x6E, 0x2E, 0x00,      # 45: cmp word [bx+2E6Eh], 0
         0x74, 0x0A,                         # 50: jz done  (+10 → byte 62)
         0x57,                               # 52: push di
         0x56,                               # 53: push si
         0x0E,                               # 54: push cs
    ]
    easy_super_suffix = [                    # bytes 58-74 (common)
         0x90, 0x90,                         # 58: NOP NOP (padding after 3-byte call)
         0x59,                               # 60: pop cx
         0x59,                               # 61: pop cx
         0x5F,                               # 62: pop di   ← done:
         0x5E,                               # 63: pop si
         0x5D,                               # 64: pop bp
         0xCB,                               # 65: retf
         0x10, 0x12, 0x2C,                   # 66: table P1: Q  E  Z
         0x16, 0x18, 0x32,                   # 69: table P2: U  O  M
         0x47, 0x49, 0x4F,                   # 72: table P3: 7  9  1
    ]
    # CALL NEAR E8 displacement: target(0x7958) - (handler_code + 58)
    for suffix, file_off in [('A',0x9868),('B',0x9924),('C',0x99E0),
                              ('D',0x9A9C),('E',0x9B38),('H',0x9C14)]:
        code_off = file_off - 0x1400
        disp = (0x7958 - (code_off + 58)) & 0xFFFF
        a.label(f'easy_super_{suffix}')
        a.db(*easy_super_prefix)
        a.db(0xE8, disp & 0xFF, (disp >> 8) & 0xFF)  # 55: call near execute_super
        a.db(*easy_super_suffix)

    # Original first 75 bytes of each combo handler (for restoration)
